...
class UpyunAction extends CommonAction{
	...
	function localUpload($filetypes=''){
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
            ...
            if($_POST['token']){
            	$tmp = trim($tmp,'/');
                $member = M("Member")->where("m_id=".$_POST['token'])->field("m_q_AccessKey,m_q_SecretKey,m_q_name,m_q_url,m_q_status")->find();
                if($member['m_q_status']){
                    $qiu_t = true;
                    // 需要填写你的 Access Key 和 Secret Key
                    $accessKey = $member['m_q_AccessKey'];
                    $secretKey = $member['m_q_SecretKey'];
                    $bucket = $member['m_q_name'];
                    $q_url =  $member['m_q_url'];
                }
                else{
                   if(C('c_q_status')){ //默认用系统自带
                        $qiu_t = true;
                        $accessKey = C('c_q_AccessKey');
                        $secretKey = C('c_q_SecretKey');
                        $bucket = C('c_q_name');
                        $q_url =  C('c_q_url');
                    }
                }
                ...
                if($qiu_t){
                            ...
                            list($ret, $err) = $uploadMgr->putFile($token, $fileName1, $tmp);
                            if ($err !== null) {
                                var_dump($err);
                                exit;
                            } else {
